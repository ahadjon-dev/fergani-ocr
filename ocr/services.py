    ...
    def process_image_extraction(image_file, language: str = 'eng', save_to_db: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if save_to_db:
                ...
                # Check for duplicate
                existing_doc = OCRDocument.objects.filter(
                    file_hash=file_hash,
                    is_deleted=False
                ).first()
                ...
                if existing_doc and existing_doc.results.exists():
                    ...
                    return {
                        ...
                        'confidence': {
                            'average_confidence': latest_result.confidence_score
                        } if latest_result.confidence_score else None,
                        'cached': True,
            ...
            if save_to_db and document:
                ...
                OCRProcessingLog.objects.create(
                    document=document,
                    level='error',
                    message=f'Failed to process {image_file.name}',
                    details={'error': str(e)}
                )
    ...
    def process_pdf_extraction(
        ...
        try:
            ...
            if pages_param:
                ...
                # Extract all pages
                result = PDFProcessor.extract_text_from_pdf(
                    pdf_file,
                    language=language,
                    use_ocr=use_ocr
                )
    ...
    def process_file(file, language: str = 'eng', save_to_db: bool = True) -> Dict[str, Any]:
        ...
        try:
            if MultiFormatService._is_pdf(file):
                ...
                return OCRService.process_image_extraction(
                    file, 
                    language=language, 
                    save_to_db=save_to_db
                )
