...
class TestOCRExtractErrorHandling(APITestCase):
    """Test error handling in OCR extraction endpoint"""
    ...
    def _create_test_image(self, size=(200, 100), format="PNG"):
        ...
        img_byte_arr.seek(0)
        return SimpleUploadedFile(
            f"test_image.{format.lower()}", img_byte_arr.read(), content_type=f"image/{format.lower()}"
        )

    @patch("ocr.views.OCRService.process_image_extraction")
    def test_tesseract_not_found_error(self, mock_process):
        """Test handling of TesseractNotFoundError"""
        mock_process.side_effect = pytesseract.TesseractNotFoundError()
        ...
        response_data = response.json()
        self.assertFalse(response_data["success"])
        self.assertIn("Tesseract OCR is not installed", response_data["error"])

    @patch("ocr.views.OCRService.process_image_extraction")
    def test_generic_processing_error(self, mock_process):
        ...
        response_data = response.json()
        self.assertFalse(response_data["success"])
        self.assertIn("Error processing image", response_data["error"])
